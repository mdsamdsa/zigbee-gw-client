...
var factory = function(proxy) {
    ...
    engine.process_zigbee_system_reset_cnf = function (msg) {
        if (msg.cmdId != Protocol.NWKMgr.nwkMgrCmdId_t.NWK_ZIGBEE_SYSTEM_RESET_CNF) {
            var str = 'process_zigbee_system_reset_cnf: Expected NWK_ZIGBEE_SYSTEM_RESET_CNF';
            logger.warn(str);
            return when.reject(new Error(str));
        }
        logger.info('process_zigbee_system_reset_cnf');
    ...
    engine.process_set_zigbee_power_mode_cnf = function (msg) {
        if (msg.cmdId != Protocol.NWKMgr.nwkMgrCmdId_t.NWK_SET_ZIGBEE_POWER_MODE_CNF) {
            var str = 'process_set_zigbee_power_mode_cnf: Expected NWK_SET_ZIGBEE_POWER_MODE_CNF';
            logger.warn(str);
            return when.reject(new Error(str));
        }
        logger.info('process_set_zigbee_power_mode_cnf');
