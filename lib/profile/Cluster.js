...
function ZCLCluster(endpoint, clusterId) {
    ...
    this.engines = endpoint.device.pan.engines;
    ...
    if (this.description.attribute) {
        this.description.attribute.forEach(function (attr) {
            ...
            var attribute = this.attributes[attr.name] = this.attributes[attributeId] = {
                attributeId: attr.id,
                name: attr.name,
                description: attr,
                ...
                read: function () {
                    if (attr.access == "w")
                        return when.reject(new Common.ZigbeeAttributeError('attribute ' + this.attributes[attributeId].name +  ' write only'));
                    return this.readAttributes(attributeId).then(function (msg) {
                ...
                write: function (value) {
                    if (attr.access == "r")
                        return when.reject(new Common.ZigbeeAttributeError('attribute ' + this.attributes[attributeId].name +  ' read only'));
                    return this.writeAttributes({id: attributeId, value: value}).then(function (msg) {
                        if (msg.attributeWriteErrorList.length == 0) {
                            attribute.value = value;
                            return msg;
                        }
                        var status = msg.attributeWriteErrorList[0].status;
                        return when.reject(new Common.ZigbeeAttributeError('write attribute ' + this.attributes[attributeId].name +  ': ' + Common.statusAttribute_toString(status), status, msg));
                    }.bind(this));
                }.bind(this)
...
ZCLCluster.prototype.sendClusterSpecificCommand = function(commandId, payload) {
    ...
    return when(this.engines.gw.attribute.send_zcl_frame_request(address, endpointIdSource, profileId, qualityOfService,
        ...
        .then(this.engines.gw.attribute.process_zcl_frame_cnf)
        .then(this.engines.wait_gateway)
        .then(this.engines.gw.attribute.process_zcl_frame_rsp_ind);
};
...
ZCLCluster.prototype.readAttributes = function() {
    ...
    return when(this.engines.gw.attribute.send_read_device_attribute_request(address, this.clusterId, attributeIds))
        .then(this.engines.gw.attribute.process_read_device_attribute_cnf)
        .then(this.engines.wait_gateway)
        .then(this.engines.gw.attribute.process_read_device_attribute_rsp_ind);
};
...
ZCLCluster.prototype.writeAttributes = function() {
    ...
    return when(this.engines.gw.attribute.send_write_device_attribute_request(address, this.clusterId, attributeRecords))
        .then(this.engines.gw.attribute.process_write_device_attribute_cnf)
        .then(this.engines.wait_gateway)
        .then(this.engines.gw.attribute.process_write_device_attribute_rsp_ind);
};
