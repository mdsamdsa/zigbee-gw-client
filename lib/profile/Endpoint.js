...
function Endpoint(device, simple_descriptor) {
    ...
    this.engines = device.pan.engines;
...
Endpoint.prototype.intUpdateGroups = function(groupList) {
    this.groups.needUpdate = false;
    for(var i = 0; i < groupList.length; i++) {
        var ind = this._get_group_index(groupList[i]);
        if (ind == -1) {
    ...
    for(var j = 0; j < this.groups.length; j++) {
        var found = false;
        var groupId = this.groups[j].groupId;
        for (var k = 0; k < groupList.length; k++) {
            if (groupId == groupList[k]) {
                found = true;
                break;
            }
        }
...
Endpoint.prototype.intUpdateScenes = function(groupId, sceneList) {
    var indGroup = this._get_group_index(groupId);
    if (indGroup != -1) {
        var group = this.groups[indGroup];
        group.scenes.needUpdate = false;
        for(var i = 0; i < sceneList.length; i++) {
            var indScene = this._get_scene_index(sceneList[i]);
            if (indScene == -1) {
                this._add_scene(groupId, sceneList[i]);
            }
        }
        for(var j = 0; j < group.scenes.length; j++) {
            var found = false;
            var sceneId = group.scenes[j].sceneId;
            for (var k = 0; k < sceneList.length; k++) {
                if (sceneId == sceneList[k]) {
                    found = true;
                    break;
                }
            }
        ...
        logger.warn('intUpdateScenes: group not found');
    }
};
...
Endpoint.prototype.addGroup = function(groupId) {
    ...
    if (ind != -1) {
        ...
        return when(this.engines.gw.group_scene.send_add_group_request(address, groupId, ''))
            .then(this.engines.gw.group_scene.process_add_group_cnf)
            .then(this.engines.wait_gateway)
            .then(this.engines.gw.group_scene.process_add_group_rsp_ind)
            .then(function() {
                this._add_group(groupId);
                return true;
            }.bind(this))
            .catch(function(err) {
                this.groups.needUpdate = true;
                throw err;
            }.bind(this));
    }
};
...
Endpoint.prototype.removeGroup = function(groupId) {
    ...
    if (ind == -1) {
        ...
        return when(this.engines.gw.group_scene.send_remove_from_group_request(address, groupId))
            .then(this.engines.gw.group_scene.process_remove_from_group_cnf)
            .then(this.engines.wait_gateway)
            .then(this.engines.gw.group_scene.process_remove_from_group_rsp_ind)
            .then(function() {
                this._remove_group(groupId);
                return true;
            }.bind(this))
...
Endpoint.prototype.storeScene = function(groupId, sceneId) {
    ...
    if (ind != -1) {
        ...
        return when(this.engines.gw.group_scene.send_store_scene_request(address, groupId, sceneId))
            .then(this.engines.gw.group_scene.process_store_scene_cnf)
            .then(this.engines.wait_gateway)
            .then(this.engines.gw.group_scene.process_store_scene_rsp_ind)
            .then(function() {
            ...
            .catch(function(err) {
                var ind = this._get_group_index(groupId);
                if (ind != -1) {
                    this.groups[ind].scenes.needUpdate = true;
                }
                return err;
            }.bind(this));
    }
...
Endpoint.prototype.removeScene = function(groupId, sceneId) {
    ...
    if (ind == -1) {
        ...
        return when(this.engines.gw.group_scene.send_remove_scene_request(address, groupId, sceneId))
            .then(this.engines.gw.group_scene.process_remove_scene_cnf)
            .then(this.engines.wait_gateway)
            .then(this.engines.gw.group_scene.process_remove_scene_rsp_ind)
            .then(function() {
                this._remove_scene(groupId, sceneId);
                return true;
            }.bind(this))
