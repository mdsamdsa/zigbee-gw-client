...
PAN.prototype.updateNetwork = function(msg) {
    if (typeof msg === 'undefined') {
        ...
        if (this.network.state == Const.NetworkState.ZIGBEE_NETWORK_STATE_READY) {
            ...
            logger.info(
                sprintf(
                    'updateNetwork: Channel 0x%02x PanID 0x%04x ExtPanID 0x%08x%08x',
                    this.network.nwk_channel,
                    this.network.pan_id,
                    this.network.ext_pan_id.high, this.network.ext_pan_id.low
                )
            );
        } else {
...
PAN.prototype.updateDevice = function(dev_info) {
    ...
    logger.info(
        sprintf(
            'updateDevice: Entry info nwkAddr 0x%04x ieeeAddr 0x%08x%08x manufactureId 0x%04x status %d numEndpoints %d',
...
PAN.prototype.getDevice = function(address) {
    if (typeof(address) == 'string') {
        address = Long.fromString(address, true);
    }
    var index = this._get_index_entry(address);
    if (index != -1) {
        return this.devices[index];
    }
};

PAN.prototype.getEndpoint = function(address) {
